...
export function updateDdevConfig(
	{ name, timezone },
	{ useCritical = true, database = null, root = ROOT, cliDir = CLI_DIR } = {},
) {
	...
	const m1Template = path.join(cliDir, 'templates', 'critical', 'config.m1.yaml');
	if (useCritical) {
		if (!fs.existsSync(m1Path) && fs.existsSync(m1Template)) {
			fs.copyFileSync(m1Template, m1Path);
		}
	} else if (fs.existsSync(m1Path)) {
		const current = fs.readFileSync(m1Path, 'utf-8');
		const template = fs.existsSync(m1Template) ? fs.readFileSync(m1Template, 'utf-8') : null;
		if (template !== null && current === template) {
			fs.rmSync(m1Path);
		} else {
			return '.ddev/config.m1.yaml';
		}
	}
